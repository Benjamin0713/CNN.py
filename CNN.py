...
# 选择模型训练再CPU还是GPU上进行 （GPU可以极大加快训练速度，但安装环境时需要安装对应的GPU版本）
...
# 如果通过代码直接下载数据集用这段代码
# mnist_path = 'D:/PYc/Zucc_AI_Lab-main/Task1_CNN/mnist/'
...
mnist_path = 'D:/PYc/Zucc_AI_Lab-main/Task1_CNN/mnist/'  # 数据集的路径

# ------------------------读取数据集------------------------ #
train_data = torchvision.datasets.MNIST(root=mnist_path, train=True, transform=torchvision.transforms.ToTensor(), download=True)
# download必须设置成True,不管之前是不是已经下载好数据集
